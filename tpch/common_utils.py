...
def execute_all(solution: str):
    ...
    with CodeTimer(name=f"Overall execution of ALL {solution} queries", unit="s"):
        for i in range(1, num_queries + 1):
            cmd = f"{sys.executable} -m {package_name}.q{i} &> results/{package_name}_q{i}.log",
            print(cmd)
            process = run(
                    f"{sys.executable} -m {package_name}.q{i} > results/{package_name}_q{i}.log 2>&1",
                    text=True,
                    shell=True
            )
            ...
            if process.returncode == 0:
                ...
                print(f"Error: Command returned non-zero exit status {process.returncode}")
