...
for name in [
    ...
    processed_table = duckdb.read_csv(
        f"tables_scale_{scale_fac}/{name}.tbl",
        header=False, sep='|', names=eval(f"h_{name}")
    )

    duckdb.sql("SELECT * from processed_table").to_parquet(
        f"tables_scale_{scale_fac}/{name}.parquet",
    )
    
    print(duckdb.sql("SELECT count(*) as rows from processed_table").df()["rows"][0])
